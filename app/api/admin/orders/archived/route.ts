...
/**
 * GET /api/admin/orders/archived - Returns all ARCHIVED orders (admin only)
 * 
 * These are orders that were delivered 15+ days ago and have been automatically archived.
 * Archived orders are read-only and provide order history for admin reporting.
 */
export async function GET() {
  try {
    ...
    const orders = await Order.find(getArchivedOrdersFilter())
